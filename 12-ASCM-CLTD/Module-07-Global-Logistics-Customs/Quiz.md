# ✏️ Module 7 Quiz: Global Logistics & Customs

> **Instructions:** 25 questions. Incoterms-heavy. Aim for 20/25.
> Each question tagged with its **Bloom's level**.

---

## Questions

### Q1. Incoterms 2020 contains how many rules? *(Remember)*
A. 7
B. 9
C. 11
D. 13

---

### Q2. Under Incoterms 2020, FOB should be used for: *(Analyze)*
A. Containerized ocean shipments
B. Bulk ocean / inland-waterway shipments where goods are loaded on a vessel
C. Air freight
D. Multimodal road/rail

---

### Q3. For a container shipped by ocean, the CORRECT Incoterm if the importer arranges main freight is: *(Apply)*
A. FOB
B. FCA
C. CIF
D. EXW only

---

### Q4. Under EXW, the seller's responsibility ends: *(Remember)*
A. At the destination port
B. At the seller's premises (buyer picks up)
C. On board the vessel
D. At customs clearance

---

### Q5. Under CIF, risk transfers from seller to buyer: *(Analyze)*
A. At the destination port
B. On board the vessel at the origin port
C. At the buyer's warehouse
D. After customs clearance

---

### Q6. The Incoterm where the seller pays EVERYTHING including import duty is: *(Remember)*
A. CIF
B. CIP
C. DPU
D. DDP

---

### Q7. The Incoterm renamed from DAT to DPU in 2020 means: *(Remember)*
A. Delivered at Place Unloaded (seller unloads)
B. Delivered at Place Unpaid
C. Direct Partial Unload
D. Deferred Payment Unloaded

---

### Q8. CIP requires the seller to obtain: *(Understand)*
A. No insurance
B. Minimum cover (Institute Cargo Clauses C)
C. All-risk cover (Institute Cargo Clauses A)
D. Cargo loss only

---

### Q9. HS codes are how many digits at the global standard level? *(Remember)*
A. 4
B. 6
C. 8
D. 10

---

### Q10. The US HTS extends HS codes to: *(Remember)*
A. 6 digits
B. 8 digits
C. 10 digits
D. 12 digits

---

### Q11. The Importer of Record (IOR) is: *(Understand)*
A. The freight forwarder
B. Legally responsible for customs declaration, duty, and compliance
C. The carrier
D. The shipping line

---

### Q12. A customs broker: *(Remember)*
A. Owns the goods
B. Files customs entries on behalf of the importer
C. Operates ships
D. Issues bills of lading

---

### Q13. A freight forwarder: *(Understand)*
A. Owns trucks and ships
B. Books transport and prepares documents, usually without taking possession of goods
C. Is the same as a customs broker
D. Operates only domestically

---

### Q14. An NVOCC: *(Remember)*
A. Owns vessels
B. Is a Non-Vessel Operating Common Carrier — issues own BL but does not own ships
C. Is a customs broker
D. Is a freight class

---

### Q15. USMCA replaced: *(Remember)*
A. NAFTA in 2020
B. CPTPP
C. RCEP
D. EU

---

### Q16. To qualify for FTA preferential treatment, goods must: *(Understand)*
A. Be cheap
B. Originate per the FTA's rules of origin, with a certificate of origin
C. Be shipped in containers
D. Be paid via Letter of Credit

---

### Q17. A Free Trade Zone (FTZ) PRIMARILY provides: *(Apply)*
A. Tax holidays
B. Duty deferral until goods enter commerce of the host country
C. Free shipping
D. Free labor

---

### Q18. Anti-dumping duty is applied when: *(Understand)*
A. Goods are dropped during shipping
B. Goods are sold in the importing market below normal market value
C. The exporter receives subsidies
D. The currency falls

---

### Q19. Letter of Credit payment is triggered by: *(Analyze)*
A. The buyer's verbal approval
B. Documents matching the LC terms
C. The carrier's notification
D. The forwarder's invoice

---

### Q20. Ad valorem duty is calculated as: *(Remember)*
A. $ per unit
B. % of declared customs value
C. Flat fee per shipment
D. Per kilogram

---

### Q21. C-TPAT (US) provides: *(Apply)*
A. Lower freight rates
B. Expedited customs treatment for security-compliant trusted traders
C. Free trade zone status
D. Duty refunds

---

### Q22. DAP and DDP differ in that DDP includes: *(Analyze)*
A. Insurance
B. Import duty paid by seller
C. Unloading at destination
D. Carrier selection by buyer

---

### Q23. A trade-show exhibitor wants to bring goods into a country temporarily without paying duty. The mechanism is: *(Apply)*
A. Open account
B. ATA Carnet (Temporary Importation under Bond)
C. Letter of Credit
D. Bonded warehouse

---

### Q24. The Incoterms order from LOWEST to HIGHEST seller responsibility starts with: *(Understand)*
A. DDP, DPU, DAP …
B. EXW, FCA, FAS, FOB …
C. CIP, CPT …
D. CIF, CFR …

---

### Q25. Under DPU, the seller's responsibility: *(Analyze)*
A. Ends at the seller's premises
B. Ends on board the vessel
C. Includes delivery AND unloading at the named place of destination
D. Includes import duty

---

## 🎯 Answers + Explanations

### Q1: **C. 11**
Incoterms 2020 has 11 rules: EXW, FCA, CPT, CIP, DAP, DPU, DDP (any mode) + FAS, FOB, CFR, CIF (ocean/inland-waterway only).

### Q2: **B. Bulk ocean where goods are loaded on a vessel**
FOB is for goods physically placed *on board*. For containers, FCA is correct.

### Q3: **B. FCA**
FCA at origin terminal/port works for containers — risk transfers when seller hands to carrier.

### Q4: **B. At seller's premises**
EXW = buyer collects from seller. Minimum seller responsibility.

### Q5: **B. On board vessel at origin**
Under all C-terms (CIF, CFR, CIP, CPT), seller pays freight to destination, but RISK transfers at origin.

### Q6: **D. DDP**
DDP = Delivered Duty Paid. Seller handles everything including import duty.

### Q7: **A. Delivered at Place Unloaded**
DPU replaced DAT in 2020. Critically, the seller unloads at destination.

### Q8: **C. All-risk (Clauses A)**
CIP requires Clauses A. CIF still requires only minimum Clauses C. (This was a 2020 change.)

### Q9: **B. 6**
HS = 6-digit global standard. Country extensions push higher.

### Q10: **C. 10 digits**
US HTS goes to 10 digits.

### Q11: **B. Legally responsible for customs**
IOR is the legal party for customs entry, duty, and compliance — typically the buyer, sometimes the seller (under DDP).

### Q12: **B. Files customs entries**
Customs broker is licensed to file entries on behalf of the importer.

### Q13: **B. Books transport, prepares docs, no goods possession**
Freight forwarders are organizers; they don't typically own assets or take title.

### Q14: **B. Non-Vessel Operating Common Carrier**
NVOCC issues its own BL but doesn't own the ship — buys space on actual carriers.

### Q15: **A. NAFTA in 2020**
US-Mexico-Canada Agreement, effective July 2020.

### Q16: **B. Originate per rules of origin + cert of origin**
FTA benefits require qualifying origin and proper documentation.

### Q17: **B. Duty deferral**
FTZs defer duty until goods leave for the host country's commerce. Also enables duty inversion.

### Q18: **B. Sold below normal value (dumping)**
Anti-dumping duty offsets predatory pricing.

### Q19: **B. Documents matching LC terms**
LCs are document-driven. A discrepancy can delay payment.

### Q20: **B. % of declared customs value**
Most common duty type globally.

### Q21: **B. Expedited customs for trusted traders**
C-TPAT, AEO, PIP all reward security investment with faster customs treatment.

### Q22: **B. Import duty paid by seller**
DAP = delivered but buyer clears customs and pays duty. DDP = seller pays duty too.

### Q23: **B. ATA Carnet (TIB)**
ATA Carnet enables temporary import without duty for samples, trade shows, professional equipment.

### Q24: **B. EXW, FCA, FAS, FOB …**
Ladder: EXW → FCA → FAS → FOB → CFR → CIF → CPT → CIP → DAP → DPU → DDP.

### Q25: **C. Delivery + unloading at destination**
DPU = the only Incoterm where the seller has to unload at destination. (Renamed from DAT in 2020 to make this explicit.)

---

## 📊 Score Yourself

- 23–25/25 → 🏆 Mastered Module 7.
- 20–22/25 → ✅ Solid.
- 16–19/25 → ⚠️ Re-read Incoterms responsibility matrix. Print it.
- <16/25 → 🔁 Restart — Incoterms is too tested to skip.

---

## 🃏 Add To Your Flashcards

- All 11 Incoterms with risk/cost split
- FOB vs FCA trap (containers)
- C-terms: pay freight to dest, but risk at origin
- DAP vs DPU vs DDP
- CIF vs CIP insurance (Clauses C vs Clauses A)
- HS (6) vs HTS (10)
- IOR / broker / forwarder / NVOCC distinctions
- USMCA, CPTPP, RCEP
- FTZ vs bonded warehouse vs TIB

---

➡️ Next: [Cheat-Sheet.md](./Cheat-Sheet.md), then [Module 8: Reverse Logistics & Sustainability](../Module-08-Reverse-Logistics-Sustainability/Reading.md)
