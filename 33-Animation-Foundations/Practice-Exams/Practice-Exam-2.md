# Practice Exam 2: Animation Foundations

> **When to take this:** After completing Modules 6–9. Target score: 24/30 (80%).
> **Time:** 35 minutes. No notes.
...
### 2. The "silhouette test" is used in both character design AND in animation production to verify:
...
### 3. A character turnaround (model sheet) shows the character in which views?

A. Close-up, medium shot, wide shot, and extreme close-up
B. Front, 3/4, side (profile), back, and 3/4 back
C. Multiple action poses demonstrating the character's full range
D. Front view only with labeled proportional measurements
...
### 5. Pixar's approach to color design begins by:
...
### 6. *Inside Out*'s progressive desaturation of San Francisco's color palette communicates:

A. The passage of seasons from summer to winter
B. The economic conditions of the city in the story's timeframe
C. Riley's deteriorating emotional state and developing depression
D. The contrast between the real world and the interior emotional world

---

### 7. A "color script" in animation production is:

A. The color reference document for a specific character's palette
B. A large-scale planning document showing how the film's palette evolves from beginning to end
C. The approved color palette for each scene delivered to lighters and compositors
D. A legal document specifying color ownership for trademark registration

---

### 8. The codec H.264 is appropriate for which use case?

A. Primary render output that will be composited and re-exported later
B. Client review, streaming delivery, and web presentation
C. Archive masters that must maintain full quality indefinitely
D. Real-time playback in animation software during production

---

### 9. The "3-2-1 Backup Rule" requires:

A. Backing up 3 times daily, on 2 drive types, once per week off-site
B. 3 copies of data, on 2 different media types, with 1 copy off-site
C. 3 separate backup applications, 2 cloud services, 1 local drive
D. Backing up every 3 hours, with 2 redundant cloud services

---

### 10. ProRes 4444 differs from ProRes 422 in that:

A. It is 44% smaller due to advanced compression algorithms
B. It supports an alpha channel (transparency), essential for VFX compositing
C. It is only compatible with Apple hardware
D. It renders 44% faster on all major GPU configurations

---

### 11. In the walk cycle, the "down position" occurs when:

A. The character's foot first touches the ground at the contact position
B. The character's body drops as weight transfers to the forward foot after contact
C. The free leg is at its lowest point during the swing phase
D. The character is in the deepest part of a crouch before a jump
...
### 13. The "pantomime test" in animation performance exercises tests:

A. Whether an animator can complete a scene within a timed deadline
B. Whether a character's emotion and narrative can be communicated through movement alone, without dialogue
C. Whether a character design works at extreme action poses
D. Whether the animator understands lip sync and phoneme mapping
...
### 15. A water balloon animated as a bouncing ball would differ from a rubber ball primarily in:

A. Moving faster per bounce due to its liquid interior creating momentum
B. Having minimal squash but maximum stretch in the downward arc
C. Extreme squash on impact and breaking (not recovering elastically) on the final contact
D. Following a flatter arc path than a rubber ball due to greater air resistance
...
### 18. In character design for animation, "designing for movement" primarily means:

A. Ensuring the character is approved for action sequences by the stunt supervisor
B. Choosing costume colors that appear vibrant in motion
C. Designing with consideration for joint clarity, detail placement, and silhouette consistency in extreme poses
D. Creating multiple character designs at different speeds for reference

---

### 19. ArtStation is particularly valuable as a portfolio platform for animation because:

A. It automatically promotes all posted animation to major studio recruiters
B. It is the dominant portfolio platform in the game art industry where animation positions are commonly posted
C. It offers more video storage than Vimeo for demo reel hosting
D. ArtStation membership is required for IATSE union applications
...
### 22. A production changes its frame rate from 24fps to 30fps mid-production. The most likely consequence is:

A. Color space issues requiring conversion across all completed scenes
B. Timing errors requiring all previously animated scenes to be re-timed
C. Incompatibility with the character rig that was built for 24fps physics
D. Render time increases of approximately 25% across all scenes

---

### 23. The "3-thumbnail rule" in storyboarding refers to:

A. Limiting each panel to no more than three characters for clarity
B. Drawing at least three composition options for each significant story beat before committing to one
C. Creating three separate versions of each animatic for director review
D. Allowing no more than three panels per storyboard page for readability

---

### 24. Toon Boom Harmony is the industry-standard software for:

A. 3D character animation for feature film production
B. Video compositing for broadcast television
C. 2D character animation for major animation studios
...
C. Increase the scale of the primary action so it reads more clearly against the secondary
D. Apply secondary action only to non-speaking characters so speaking characters are unaffected

---

### 26. In the bouncing ball exercise, successive bounces should be:

A. Identical in height and timing to create regular rhythm
B. Progressively larger as the ball gains energy from elastic recovery
C. Progressively smaller and faster as the ball loses energy through impact and friction
D. Alternating between small and large to create visual interest

---

### 27. The recommended opening shot for a demo reel aimed at a character animation position is:
...
### 28. Blue is most commonly associated with which character personality trait in animation color design?

A. Warmth and optimism
B. Menace and villainy
C. Calm, trustworthiness, or melancholy/isolation
D. Youthful energy and playfulness
