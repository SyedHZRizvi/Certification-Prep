---
permalink: /35-Motion-Graphics-UI-Animation/Module-09-Sound-Design-Motion/Quiz/
...
### Q2.
In the three-level model of sound in motion graphics, which layer serves as the emotional foundation?
...
### Q5.
When a visual event is close to but not exactly on a transient, it is better practice to:

A. Delay the visual event to match the audio precisely  
B. Move the visual event earlier (lead the audio) rather than later (lag the audio)  
C. Move the audio transient to match the visual event  
D. Add a cross-fade of 2 frames to smooth the mismatch  

---

### Q6.
"Royalty-free" music licensing means:
...
### Q7.
Epidemic Sound is best described as:

A. A free SFX library with Creative Commons-licensed sounds  
B. A subscription service for music and SFX with licenses for personal and commercial content creation  
C. A stock video platform that includes music  
D. A sound plugin for After Effects  

---

### Q8.
The "sweetening" phase of sound design in broadcast involves:

A. Increasing the music volume by 6dB to make the piece more emotional  
B. Adding subtle SFX that enhance the audio environment without calling conscious attention to themselves  
C. Removing all SFX and relying on music alone  
D. Adding a low-frequency hum to fill frequency gaps  

---

### Q9.
In After Effects, which feature converts audio amplitude data into keyframes that can drive animations?

A. Audio Waveform Display  
B. Layer > Audio > Convert Audio to Keyframes  
C. Effect > Audio > Amplitude Keyframer  
D. Expression: `audioAmplitude.value`  

---

### Q10.
The Audio Spectrum effect in After Effects is found under:
...
### Q11.
For an audio spectrum visualizer with circular display, what must you first create on the layer before applying the Audio Spectrum effect?

A. A camera layer  
B. A mask path in the shape of an ellipse  
C. A Track Matte  
...
A. All animations should use the same musical key  
B. High-tempo music requires looping animations  
C. The emotional quality of the audio and the animation vocabulary should match (or deliberately contrast for effect)  
D. Animation speed should match the musical tempo in BPM  

---

### Q13.
Which audio platform is noted for offering a massive SFX-focused library as its primary offering?
...
### Q14.
A sync license allows:

A. Using music in a commercial without paying royalties  
B. Using the published musical composition in synchronization with video content  
C. Distributing a recorded version of a song  
D. Using a song in any digital medium  

---

### Q15.
Which order represents the correct workflow for sound design in a motion graphics project?

A. Add SFX first, then choose music, then sync to beats  
B. Choose music first → mark beats → build animation to beat structure → add SFX last → mix  
C. Build the animation completely, then find music that matches  
D. Build SFX and animation simultaneously, then add music as a final layer  

---

### Q16.
For a corporate motion graphics piece with voice-over narration, the approximate relative volume mix is:

A. Music 0dB, SFX -6dB, voice-over -3dB  
B. Music -6 to -12dB, SFX -3 to 0dB, voice-over 0dB (loudest)  
C. Music 0dB, voice-over 0dB, SFX -6dB  
D. All elements at equal volume for clarity  

---

### Q17.
Which of the following audio characteristics matches fast, linear animation with high contrast (as described in the Tone of Motion matrix)?

A. Ambient/minimal audio with slow, textural motion  
B. High-energy electronic audio with fast, sharp, linear-eased motion  
C. Orchestral/dramatic audio with arcing motion paths  
D. Playful/quirky audio with spring easing  

---

### Q18.
A visual hit point in motion design refers to:

A. A keyframe that has been manually set to "impact" timing  
B. A moment where a visual event coincides exactly with an audio event (transient or musical accent)  
C. A type of shape animation that creates a visual impact effect  
D. The first frame of an animation that is most important  

---

### Q19.
The After Effects Audio Spectrum "Frequency Bands" parameter controls:
...
### Q20.
Which music licensing type is required to use the specific recorded version of a song (not just the composition) in a video?
...
C. Create cognitive dissonance that forces viewer attention and emotional engagement  
D. Compensate for poor animation quality  

---

### Q24.
Freesound.org provides audio under Creative Commons licenses. Before using a Freesound clip in a commercial project, you must:
