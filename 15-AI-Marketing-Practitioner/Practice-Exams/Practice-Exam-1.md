...
### 1. The 5 W's of a campaign brief are:
A. Who, What, When, Where, Why
B. Whose, What, Why, Where, When
C. Why, How, When, What, Where
D. Who, What, Why, Where, How

### 2. A "guardrail KPI" is:
A. The metric you most want to hit
B. A vanity metric
C. The metric that, if breached, triggers a pause
D. The primary metric
...
### 5. Maximum headlines on a Responsive Search Ad:
A. 15
B. 10
C. 5
D. 30

### 6. Quality Score components are:
...
### 7. Ad Rank formula:
A. Bid × Quality Score + Extension impact
B. QS only
C. Bid only
D. Bid + QS + Position

### 8. The "Include Display Network" default on a new Search campaign should be:
A. Unchecked for pure Search
B. Left checked (default)
C. Set to ad-group level
D. Required

### 9. Performance Max runs on:
A. Search only
B. YouTube only
C. Search + Display + YouTube + Discover + Gmail + Maps simultaneously
D. Just YouTube

### 10. The MOST important PMax setting to prevent brand cannibalization:
A. Final URL Expansion
B. Geo
C. Daily Budget
D. Brand Exclusions

### 11. An audience signal in PMax is:
A. Hard targeting that restricts who can see
B. Unchangeable
C. Required
D. A HINT to the algorithm

### 12. Skippable in-stream YouTube ads are paid when:
A. Ad starts
B. Viewer clicks
C. Viewer reaches 30 sec OR full play
D. Ad completes regardless

### 13. Non-skippable YouTube in-stream max length:
A. 6 sec
B. 15 sec
C. 30 sec
D. 60 sec

### 14. The 3-second hook test for short-form video:
A. Hook attention with question/pattern/claim/demo/stat in first 3 seconds
B. Logo in first 3 seconds
C. Show product within 3 seconds
D. Pause for 3 sec

### 15. Best Meta setup for a Shopify store launching in 2026:
A. Personal Facebook ad account
B. Pixel only
C. Business Manager + Pixel + CAPI + Domain Verified + AEM events ranked
D. No tracking needed

### 16. Why is CAPI critical after iOS 14.5?
A. Browser Pixel is blocked by 75% of iOS users via ATT; CAPI (server-side) bypasses this
B. It's faster
C. Apple requires it
D. It's cheaper

### 17. Current default Meta attribution window (post iOS 14):
A. 28-day click + 1-day view
B. Last-touch only
C. 7-day click + 1-day view
D. 90-day click

### 18. Lookalike Audience minimum recommended seed size:
A. 100
B. 100,000+
C. 1,000+ (ideally 5,000+)
D. 1M+

### 19. Advantage+ Shopping Campaigns (ASC) work best when:
A. Less than 10 daily purchases
B. No catalog
C. Solo product, no catalog
D. 50+ daily purchases AND a catalog AND $50+/day budget

### 20. The "Existing Customer Budget" cap in ASC lets you:
A. Limit spend on existing customers (forcing prospecting)
B. Cap total spend
C. Set CPA target
D. Get refunds

### 21. Dynamic Creative Testing (DCT) typically tests across:
...
### 24. TikTok Spark Ads are:
A. Premium-only ad format
B. Sponsored boosts of existing organic TikTok videos (yours or a creator's via Spark Code)
C. AR filters
D. App-open takeovers

### 25. TikTok's first-impression hook window (vs Meta's):
A. 5 seconds (same)
B. 15 seconds
C. 1.5-3 seconds (faster than Meta)
D. 30 seconds
...
A. Sponsored Content
B. Thought-Leadership Ads (boost individual employee posts)
C. Text Ads
D. Dynamic Ads

### 28. LinkedIn Matched Audiences "Account List Upload" enables:
A. Building lookalikes
B. Targeting specific named companies (ABM foundation)
C. Sending InMail directly
D. Uploading creative

### 29. LinkedIn Lead Gen Forms auto-fill:
A. Email only
B. Nothing
C. Credit card info
D. Profile data (name + email + company + title) from user's LinkedIn profile

### 30. For a B2B SaaS targeting "VP Marketing at companies 200+", the right primary platform is:
A. TikTok
B. Snapchat
C. Pinterest
D. LinkedIn

### 31. Comparing LinkedIn CPCs to Meta CPCs and calling LinkedIn "too expensive" is:
A. Correct
...
A. 5%
B. 38% (from 67 to ~42 days)
C. 80%
D. Negligible

### 33. The proposition in a creative brief should be:
A. Single message in 12 words or fewer
B. A list of 3-5 benefits
C. Full feature list
...
D. Programmatic Lead Gen

### 35. To improve Quality Score, you can improve:
A. CTR + Ad Relevance + Landing Page Experience
B. Bid only
C. Budget only
D. Geo

### 36. The Allbirds Google Ads case study used how many campaigns total?
A. 1
B. 5 (Brand, Generic, Sustainable, Running, Retargeting)
C. 50+
D. 100+

### 37. The right way to set up a brand-new TikTok ad campaign for a DTC apparel store:
A. Pangle + News Feed only
B. TikTok only with Spark Ads via UGC creators, vertical 9:16, sound-on hook
C. Horizontal only
D. CPC bidding required

### 38. A "Spark Code" is:
A. An authorization code from a creator letting you boost their organic TikTok as an ad
B. A discount code
C. A TikTok API key
D. A login token

### 39. The Notion ABM case study showed enterprise pipeline grew by approximately:
A. 10% per year
B. 10x in 12 months (driven by LinkedIn account lists + TLAs)
C. 2x
D. Flat

### 40. A campaign brief should typically be:
A. As long as needed
B. 10 pages
C. 50 pages
D. 1-2 pages
...
**Q5. Answer: A.** Why A is correct: Responsive Search Ads allow up to 15 headlines (and 4 descriptions). Why wrong: B is the historic ETA limit; C is too low; D is invented. Exam-takeaway: 15 + 4 is the RSA spec.
...
**Q13. Answer: B.** Why B is correct: Non-skippable in-stream YouTube ads are capped at 15 seconds. Why wrong: A is bumper-ad length (6s); C is the old non-skip limit; D is well over. Exam-takeaway: Non-skippable = 15s max; bumper = 6s.

**Q14. Answer: A.** Why A is correct: The 3-second hook test means using question/pattern/claim/demo/stat to capture attention in the first 3 seconds. Why wrong: B/C describe logo-first or product-first (proven to underperform); D is opposite. Exam-takeaway: Earn attention before showing brand or product.

**Q15. Answer: C.** Why C is correct: Day-0 Meta setup requires Business Manager + Pixel + CAPI + Domain Verified + AEM-ranked events. Why wrong: A is the lose-control trap; B is pre-iOS-14.5 setup; D guarantees broken attribution. Exam-takeaway: Without this defensive stack, your tracking is broken from minute one.
...
**Q19. Answer: D.** Why D is correct: Advantage+ Shopping Campaigns (ASC) need 50+ daily purchases AND a catalog AND $50+/day budget to escape volatility. Why wrong: A-C describe under-scaled scenarios where ASC starves. Exam-takeaway: ASC = scale tool, not starter tool.

**Q20. Answer: A.** Why A is correct: The Existing Customer Budget cap limits spend on existing customers (forcing the algorithm to find new ones). Why wrong: B/C describe other settings; D is invented. Exam-takeaway: ECB cap = prospecting forcing function.

**Q21. Answer: B.** Why B is correct: Dynamic Creative Testing tests across Hook × Body × CTA (typically 4×4×4 = 64 variants). Why wrong: A is split-testing (different); C is dayparting (not creative); D conflates bidding with creative. Exam-takeaway: DCT axes = creative components, not audience/bidding axes.

**Q22. Answer: C.** Why C is correct: 100 impressions is noise; you need ~1,000+ per variant before performance signal stabilizes. Why wrong: A/B/D would all lead to killing winners early. Exam-takeaway: ~1K impressions/variant is the minimum sample-size floor.
...
**Q37. Answer: B.** Why B is correct: TikTok DTC apparel = Spark Ads via UGC creators, vertical 9:16, sound-on hook. Why wrong: A includes Pangle / News Feed (non-TikTok inventory that underperforms); C is wrong aspect; D is wrong bidding. Exam-takeaway: TikTok needs native vertical UGC, not polished cross-platform creative.

**Q38. Answer: A.** Why A is correct: A Spark Code is an authorization code from a creator letting you boost their organic TikTok as an ad. Why wrong: B is unrelated; C is invented; D conflates with API auth tokens. Exam-takeaway: Spark Code = creator-issued ad permission.
...
| Score | Verdict |
|-------|---------|
| 36-40 | 🏆 Crushing it. Take Practice Exam 2 next. |
| 32-35 | ✅ Solid. Review wrong answers. |
| 25-31 | ⚠️ Re-read Modules 2-4 weak spots. Re-test. |
| < 25  | 🔁 Restart from Module 1. |
...
➡️ When ready: [Practice Exam 2](./Practice-Exam-2.md) (after finishing Modules 6-9)
