...
The Microsoft Certified: Identity and Access Administrator Associate (SC-300) is the credential for the engineer who owns the most important security control in any modern enterprise, **identity**. In a Zero Trust world, the perimeter is gone; identity *is* the perimeter. SC-300 holders are the practitioners trusted with Microsoft Entra ID tenants, Conditional Access policies, MFA (Multi-Factor Authentication) / passwordless rollouts, Identity Protection, Privileged Identity Management, and the External ID and B2B (Business-to-Business) / B2C (Business-to-Consumer) governance that decides who gets into your tenant and what they can do once they're in.
...
- **IAM (Identity and Access Management) Engineer / Identity Engineer** ($110K–$155K), design and operate identity at enterprise scale
- **Cloud Security Engineer (identity focus)** ($115K–$160K), Conditional Access, PIM (Product Information Management), Identity Protection
...
- **CISO (Chief Information Security Officer)-track Identity Lead** ($160K–$240K + bonus), typical at Fortune 500 with multi-tenant footprint
...
- **Real-world case studies**, the SolarWinds 2020 SAML-token forgery technique (Golden SAML), the Maersk NotPetya AD (Active Directory) recovery, an M&A (Mergers and Acquisitions) identity-merge scenario, the 2023 Storm-0558 token-signing-key compromise
...
- Stand up and brand a Microsoft Entra ID tenant, add custom domains, and pick the right license SKU (Stock Keeping Unit) (Free, P1, P2, External ID)
...
- Register applications (OIDC, SAML, WS-Fed), wire up SSO (Single Sign-On), App Proxy, admin consent workflow, and app role assignments
...
| 6 | [Identity Governance & PIM](./Module-06-Governance-PIM/Reading.md) | 4 hrs | PIM eligible vs active, JIT (Just-In-Time) activation, access reviews, entitlement mgmt (access packages, catalogs), Lifecycle Workflows |
...
Week 4: Module 5 (app registration + SSO)
        → Register an OIDC app + a SAML SaaS (Software as a Service) app from the gallery
...
4. ❌ **Mixed up directory roles, Entra roles, and Azure RBAC (Role-Based Access Control)**, three different role systems. They share names but not scopes.
