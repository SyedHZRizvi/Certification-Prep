...
> **Why this module matters:** Azure Arc is the headline AZ-801 topic. It's the *bridge* that pulls every on-prem, multicloud, and edge resource into Azure Resource Manager — so the same Policy, RBAC, Monitor, Defender, and Update Management tools that govern your Azure-native workloads also govern your Windows Server fleet wherever it lives. Master Arc and you've unlocked roughly 15% of AZ-801 directly *and* made every later module (Monitor, Defender, Backup) dramatically easier.
...
> - On-prem AD and Entra ID hybrid — [Modules 1 & 2](../Module-01-Active-Directory/Reading.md)
> - PowerShell basics + outbound HTTPS firewall concepts
>
...
- **AWS:** 90 EC2 instances (Windows Server inherited from a 2018 acquisition)
...
Azure Arc is a **control plane** that extends Azure Resource Manager (ARM) to resources outside Azure: on-prem servers, AWS EC2, GCP Compute Engine, edge devices, even other ARM-targets.
...
| **Extension hosting** | Hosts Azure VM extensions (Monitor Agent, Defender, Update Mgmt, Run Command, etc.) |
...
🔥 **Closed networks** (no internet) can use **Azure Arc Private Link** so the agent talks to a Private Endpoint on your VNet, which forwards to Azure over your existing ExpressRoute/VPN.
...
Connects **any** existing Kubernetes cluster (AKS, EKS, GKE, OpenShift, bare-metal Rancher, K3s on a Raspberry Pi) to Azure as `Microsoft.Kubernetes/connectedClusters`.
...
Run a script (PowerShell or Bash) on an Arc-enabled server from Azure portal/CLI without needing inbound SSH/RDP. Uses the Connected Machine agent as the execution channel.
...
| Onboard via GPO at scale | One-time push, ongoing self-heal |
...
**Discussion (Socratic).**
- **Q1.** Microsoft's GPO-based onboarding pushed Arc to 95% of AD-joined Windows in 90 days. Build the rollout plan for a 6,000-server enterprise: who approves the GPO link, what's the rollback plan, and how do you handle servers that fail to onboard (DNS resolution, firewall whitelisting)?
- **Q2.** Arc-enabled servers cost ~$6/server/month (depending on Defender SKU). For a 500-server SMB, that's $36K/year. Build the business case vs (a) hand-managed WSUS + standalone Sysmon + per-server monitoring tools, and (b) lift-and-shift everything to Azure VMs.
