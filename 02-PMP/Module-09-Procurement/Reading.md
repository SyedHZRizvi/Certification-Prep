# PMP Module 9: Procurement Management 🛒

> **Why this module matters:** ~5-10 questions. Procurement is contract-heavy. Know contract types and the procurement process flow.

---

## 🎯 What Is Procurement?

Procurement = buying or contracting **outside** your organization for goods or services.

In project terms:
- 🛠️ Hiring vendors
- 🤝 Sub-contracting work
- 🏗️ Renting equipment
- 📦 Buying materials
- 💼 Engaging consultants

---

## 🔄 The 3 Procurement Processes

### 1. Plan Procurement Management
- 📋 What to buy, how, when
- Output: **Procurement Management Plan**, **Procurement Statement of Work (SOW)**

### 2. Conduct Procurements
- 📨 Send out RFx (RFP, RFQ, RFI)
- 🤝 Evaluate bids
- 📝 Sign contracts

### 3. Control Procurements
- 👀 Monitor vendor performance
- 🔄 Manage changes
- 💵 Process payments
- 🏁 Close out (after delivery)

---

## 📜 RFx — Request for What?

| RFx | What It Asks | When |
|-----|--------------|------|
| **RFI** (Request for Information) | "What can you do?" | Early — gathering info |
| **RFP** (Request for Proposal) | "How would you do it?" | Solution unclear |
| **RFQ** (Request for Quotation) | "How much will you charge?" | Solution clear, just price |
| **IFB / RFB** (Invitation/Request for Bid) | Sealed bids | Government, fixed scope |

🎯 **Exam pattern:** "Buyer needs to compare prices for a well-defined product." → **RFQ.**

---

## 📋 Contract Types (CRITICAL!)

3 main families. Know each:

### 1. Fixed-Price (FP) Contracts
**Vendor takes the risk.**

| Type | What |
|------|------|
| **FFP (Firm Fixed-Price)** | Single fixed price; vendor risk if costs exceed |
| **FPIF (Fixed-Price Incentive Fee)** | Fixed + bonus for performance |
| **FP-EPA (Fixed-Price w/ Economic Price Adjustment)** | Adjusts for inflation, exchange rates |

🎯 Use when: scope is **clear and stable**.

### 2. Cost-Reimbursable (CR) Contracts
**Buyer takes the risk** (you pay all costs + a fee).

| Type | What |
|------|------|
| **CPFF (Cost Plus Fixed Fee)** | Costs + fixed fee |
| **CPIF (Cost Plus Incentive Fee)** | Costs + bonus for performance |
| **CPAF (Cost Plus Award Fee)** | Costs + subjective award |

🎯 Use when: scope is **uncertain or evolving**.

### 3. Time & Materials (T&M) Contracts
**Hybrid** — pay for time + materials.

🎯 Use when: small, short-term, or staffing arrangements.

---

## 💰 Risk Allocation by Contract Type

```
HIGH BUYER RISK ←————————————————→ HIGH SELLER RISK
  
  Cost+ (CPFF)    T&M    Fixed Price (FFP)
```

🎯 **Memory:** "Cost-plus = customer pays. Fixed-price = vendor pays (if over)."

🎯 **Exam pattern:** "Project has well-defined scope. Best contract type for buyer?" → **Fixed-Price.**
"Scope is highly uncertain. Best contract for buyer?" → **Cost-Reimbursable** (despite higher risk, you avoid vendor padding).

---

## 📜 SOW (Statement of Work)

Describes the work to be procured:
- Scope
- Deliverables
- Acceptance criteria
- Schedule

3 types:
- **Performance SOW** — describe end results (vendor figures out how)
- **Functional SOW** — performance + features
- **Design SOW** — describe exactly how (rare in agile)

---

## 🎯 Make-or-Buy Analysis

Decision: build internally or buy externally?

Considerations:
- Cost
- Skills available
- Strategic value (don't outsource core competence)
- Speed
- Risk

---

## 📨 The Procurement Process Flow

```
Plan → SOW + RFP → Send to vendors → Receive bids
   ↓
Evaluate (price, qualifications, references)
   ↓
Negotiate
   ↓
Sign contract
   ↓
Manage performance during execution
   ↓
Close out (formal acceptance, lessons learned)
```

---

## 🏁 Contract Closure

When the work is done:
1. ✅ Verify all deliverables met
2. 📜 Formal acceptance
3. 💵 Final payments
4. 📚 Lessons learned (about vendor performance)
5. 📦 Archive contract

---

## 🤝 Centralized vs Decentralized Procurement

- **Centralized** — one office handles all procurement (consistent, expert)
- **Decentralized** — each PM handles theirs (faster, may lack expertise)

---

## 📊 Procurement in Agile

Agile contracts are tricky because scope evolves:
- 💼 **T&M for development**
- 💼 **Capped T&M** (cap on hours/dollars)
- 💼 **Iterative fixed-price** (each sprint = new fixed-price contract)
- 💼 **Money for nothing, change for free** (innovative agile contract structure)

🎯 **Insight:** Pure Fixed-Price contracts often clash with agile. Hybrid approaches work best.

---

## ⚠️ Common Procurement Anti-Patterns

- ❌ Going with lowest bid without quality consideration
- ❌ No SOW (vendor delivers wrong thing)
- ❌ Skipping evaluation criteria
- ❌ Vendor contracts but no PM involvement during execution
- ❌ Closing contract without formal acceptance

---

## 🎓 Key Terms

| Term | Definition |
|------|------------|
| **SOW** | Statement of Work |
| **RFI** | Request for Information |
| **RFP** | Request for Proposal |
| **RFQ** | Request for Quotation |
| **FFP** | Firm Fixed-Price |
| **CPFF** | Cost Plus Fixed Fee |
| **CPIF** | Cost Plus Incentive Fee |
| **T&M** | Time and Materials |
| **Make-or-Buy** | Internal vs external decision |

---

## ✅ Module 9 Summary

You now know:
- 🔄 3 procurement processes (Plan, Conduct, Control)
- 📜 RFx types (RFI, RFP, RFQ)
- 📋 Contract families: FP (vendor risk), CR (buyer risk), T&M
- 📊 SOW types
- 🎯 Make-or-buy analysis

**Next:**
1. 🎥 [Videos.md](./Videos.md)
2. ✏️ [Quiz.md](./Quiz.md)
3. 📋 [Cheat-Sheet.md](./Cheat-Sheet.md)
4. ➡️ [Module 10: Stakeholders](../Module-10-Stakeholders/Reading.md)

---

## 🎓 Cornell/Harvard/Stanford Elevation Layer

### Prerequisites for this module

> **Prerequisites for this module.** Before starting, you should be comfortable with:
> - **Project Risk basics** — Module 07 introduces the risk-transfer concept that underlies contract families.
> - **Business Case + Make-or-Buy fundamentals** — covered in [Module 04: Business Environment](../Module-04-Business-Environment/Reading.md).
> - **Optional cross-course preparation:** [13-ISM-CPSM Module 05: Contract Management](../../13-ISM-CPSM/Module-05-Contract-Management/Reading.md) is the deep-dive on contract types and supplier management.
>
> If any of these are shaky, pause and review before continuing.

### Where this leads

> **Where this leads.**
> - **Inside this course:** [Module 07 — Risk](../Module-07-Risk/Reading.md) discusses transfer-via-contract; [Module 10 — Stakeholders](../Module-10-Stakeholders/Reading.md) covers vendor-as-stakeholder.
> - **Cross-course:** [13-ISM-CPSM Module 05](../../13-ISM-CPSM/Module-05-Contract-Management/Reading.md) is the supply-management deep dive; [10-ASCM-CSCP Module 09](../../10-ASCM-CSCP/Module-09-Strategic-Sourcing-Procurement/Reading.md) extends to strategic sourcing.
> - **Practice:** [Practice Exam 1](../Practice-Exams/Practice-Exam-1.md), [Practice Exam 2](../Practice-Exams/Practice-Exam-2.md), and [Final Mock Exam](../Practice-Exams/Final-Mock-Exam.md) each draw ~5–10 questions on procurement and contracts.

---

## 📊 Case Study — Denver International Airport Automated Baggage System (1989–1995)

**Situation.** Denver International Airport (DIA) was a $4.8B mega-project to replace Stapleton International Airport. The Automated Baggage System (ABS) was one of the most ambitious civil-infrastructure software projects ever attempted: 26 miles of conveyor track, 4,000 telecars, 5,000+ electric eyes, and 56 barcode scanners — designed to handle bags airport-wide. The original contract was awarded to BAE Automated Systems in 1992, mid-construction. The intended scope was a single-airline (United) inbound + outbound automated handling.

**Decision.** Procurement decisions cascaded into disaster: (1) the city of Denver issued a request for a full-airport automated system *after* construction had begun, with most of the airport's physical layout already locked in (a Design-SOW situation where the design was incomplete); (2) BAE was awarded a Firm Fixed-Price contract for a system whose requirements were not yet stable — bypassing the principle that FFP requires defined scope; (3) the contract did not include a deferred-acceptance milestone schedule with phased payments tied to subsystem performance; (4) other airlines (Continental, then-existing carriers) demanded coverage they hadn't paid for, expanding scope without formal change orders; (5) BAE warned in writing that timelines were not feasible, but the warnings were treated as negotiating posture by Denver's project leadership rather than as engineering reality.

**Outcome.** The ABS opened in February 1995 — **16 months late**. During testing, the system **destroyed bags, mis-routed bags, and crashed repeatedly**. United Airlines reverted to a manual backup. The system was scaled back to United-outbound-only and was finally **decommissioned entirely in 2005**, with the airport paying ~$1M/month to keep the unused infrastructure powered until shutdown. Total cost overruns for the baggage system alone exceeded **$560 million** (well over 100% of the original ABS budget). Direct losses (airport opening delays at $1M/day, etc.) added at least **$2 billion** more. The 1996 GAO report (GAO/RCED-96-114) and Calleam Consulting's 2008 case study became the canonical "how *not* to procure a complex system" examples in software-engineering and PM curricula worldwide.

**Lesson for the exam / for practitioners.** Denver Airport is the textbook example of *contract-family mismatch*. PMBOK 7 teaches: FFP requires stable scope; uncertain scope demands Cost-Reimbursable or T&M structures. The city of Denver chose FFP for a system whose scope was undefined and changing. The right answer to *"Project has uncertain scope, but a fixed-price contract was signed — what should the PM do?"* is *not* "renegotiate to FFP" — it is *"renegotiate to a hybrid (e.g., capped T&M, milestone-based, or Cost-Plus with incentive fees)"* AND *"engage the vendor in joint planning to scope-stabilize"*. The exam tests this distinction repeatedly. **Procurement is not contract paperwork — it is the formalized expression of project risk allocation.**

**Discussion (Socratic).**
- Q1: BAE warned in writing that the timeline was infeasible. The city treated the warning as posturing. From PMI's Code of Ethics perspective, what should BAE's PM have done next? What should Denver's PM have done?
- Q2: The procurement officer chose FFP partly because public-sector procurement law often favors fixed-price (auditability, fairness to bidders). When does fiduciary procurement law conflict with project-management reality? Defend an answer.
- Q3: Design a procurement framework for a 2024 project of similar complexity (e.g., a city-wide smart-traffic system) that incorporates the Denver lessons — what contract structure, payment schedule, and governance would you require?

---

## 💬 Discussion — Socratic prompts

1. **Contract-family selection.** A vendor wants Firm-Fixed-Price for a 3-year IoT deployment in 50 stores. The buyer's scope is still evolving as user research continues. Build the case for: (a) honoring the FFP request; (b) refusing and going CPIF; (c) a hybrid. Defend one.
2. **Money-for-nothing change-for-free contract.** This unusual agile contract structure pays the vendor for un-delivered backlog and lets the buyer trade items for "free." Is it the right answer for any context, or is it always a gimmick? Cite specific use cases.
3. **Centralized vs decentralized procurement.** A 4,000-person enterprise has centralized procurement. PMs complain about slowness. Build the strongest case for decentralization AND for keeping centralization. Cite PMBOK 7's Tailoring principle.
4. **Vendor lock-in.** Make-or-buy decisions sometimes seem cheaper externally but create long-term lock-in (think AWS vs on-premise). How do you incorporate strategic-value and exit-cost considerations into the decision?
5. **RFx sequencing.** A buyer issues RFP after RFI after RFQ. Is the sequence right, or could it be compressed? Cite PMBOK 7 and a real-world reasoning chain.

---

## 📚 Named-source citations (this module)

| Framework / claim | Originator | Year | Publication |
|---|---|---|---|
| Procurement Management Processes (Plan / Conduct / Control) | Project Management Institute | 2017; reaffirmed 2021 | PMBOK 6 §12; PMBOK 7 §2.4 |
| Contract families (FP / CR / T&M) | US DOD; codified by PMI | various; PMBOK 1996+ | PMBOK 6 §12.2 |
| Fixed-Price + variants (FFP / FPIF / FP-EPA) | Federal Acquisition Regulation (FAR) | 1984 (current FAR) | FAR Part 16 |
| Cost-Reimbursable variants (CPFF / CPIF / CPAF) | Federal Acquisition Regulation (FAR) | 1984; updated annually | FAR Part 16 |
| Statement of Work (SOW) types (Performance / Functional / Design) | Federal Acquisition Regulation (FAR) | 1984 (current) | FAR 37.602 |
| RFx terminology (RFI / RFP / RFQ / IFB) | US federal procurement practice | 1970s onward | FAR; widely adopted commercially |
| Make-or-Buy analysis | Williamson, Oliver | 1985 | *The Economic Institutions of Capitalism*, Free Press (transaction-cost economics) |
| Agile-specific contract structures (capped T&M, money-for-nothing) | Sutherland, Jeff & Schwaber, Ken | 2010s | various Scrum.org publications |
| Centralized vs Decentralized Procurement | Monczka, Handfield, et al. | 2020 | *Purchasing and Supply Chain Management* (7th ed.), Cengage |
| Denver Airport ABS case | US Government Accountability Office | 1996 | GAO/RCED-96-114 *Automated Baggage System Update* |
| Denver Airport ABS detailed post-mortem | Calleam Consulting | 2008 | *Case Study: Denver International Airport Baggage Handling System* |
| ISO 31000 risk-in-procurement framing | International Organization for Standardization | 2018 | ISO 31000:2018 |
| Standish CHAOS Report (vendor-failure data) | Standish Group | 2024 | *CHAOS Report 2024* |
