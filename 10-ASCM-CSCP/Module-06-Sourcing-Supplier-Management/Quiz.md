# ✏️ Module 6 Quiz: Sourcing & Supplier Management

> **Instructions:** Answer all 25 questions WITHOUT looking at the reading.
> Then check your answers below. Aim for 20/25 minimum.

---

## Questions

### Q1. A buyer wants market intelligence on capabilities of potential suppliers. The correct document is:
A. RFI
B. RFP
C. RFQ
D. PO

---

### Q2. A custom electric motor used in only one of your product platforms is in the Kraljic quadrant:
A. Routine
B. Leverage
C. Bottleneck
D. Strategic

---

### Q3. The 7-step strategic sourcing process starts with:
A. Negotiate
B. Spend analysis
C. RFP
D. Supplier scorecard

---

### Q4. RFQ is BEST used when:
A. Capabilities are unknown
B. The buyer has a well-defined spec and wants price quotes
C. The buyer wants comprehensive solutions
D. The buyer plans a multi-year strategic alliance

---

### Q5. TCO differs from price because TCO includes:
A. Only purchase price
B. Operating, maintenance, and disposal across lifecycle
C. Setup time only
D. Carrying cost only

---

### Q6. A reverse auction is generally INAPPROPRIATE for:
A. Office supplies
B. Bulk packaging
C. Strategic custom components requiring deep collaboration
D. Standard cabling

---

### Q7. Office paper purchased through an e-catalog by 200 buyers across the firm is BEST classified as:
A. Strategic
B. Leverage
C. Bottleneck
D. Routine

---

### Q8. In a Kraljic "bottleneck" item (high supply risk, low profit impact), the BEST sourcing strategy is:
A. Lowest-price competitive bidding
B. Ensure supply via dual-sourcing or safety stock
C. Eliminate the supplier
D. Centralize spend

---

### Q9. A cost-plus contract transfers MORE risk to:
A. The buyer
B. The supplier
C. The freight forwarder
D. The end customer

---

### Q10. SLA stands for:
A. Supplier Liability Agreement
B. Service-Level Agreement
C. Strategic Liaison Authority
D. Supply Lane Audit

---

### Q11. BATNA refers to:
A. Best Alternative To a Negotiated Agreement
B. Buyer's Aggregate Total Net Asset
C. Backup-Allocated TCO Negotiation Aid
D. Backup Asset Tax Number Agreement

---

### Q12. A supplier scorecard typically gives the HIGHEST weight to:
A. Quality and on-time delivery
B. Phone responsiveness
C. Number of staff
D. Number of certifications

---

### Q13. SRM at the "strategic alliance" level typically includes:
A. Pure price competition
B. Joint R&D, shared scorecards, sometimes equity
C. One-off transactions
D. No information sharing

---

### Q14. A buyer who must purchase a single-supplier specialty pigment with no substitute should:
A. Reverse auction every quarter
B. Develop alternates, increase safety stock, and qualify a second source
C. Drop the supplier
D. Issue an RFI

---

### Q15. Acquisition + operating + maintenance + disposal best describes:
A. EOQ
B. TCO
C. ROP
D. CPFR

---

### Q16. A leverage item in Kraljic terms is BEST sourced via:
A. Long-term strategic alliance
B. Competitive bidding / reverse auction
C. Single-source vertical integration
D. Sole-source manual catalog

---

### Q17. Which is NOT a strategic sourcing step?
A. Analyze spend
B. Solicit suppliers (RFx)
C. Negotiate and award
D. Compute EOQ

---

### Q18. Dickson's classic top supplier-selection criterion was:
A. Lowest price
B. Quality
C. Geographic proximity
D. Financial leverage

---

### Q19. MFN (most favored nation) clause means:
A. The supplier promises buyer pricing at least as good as any other customer
B. Most freight nationally
C. Multi-firm negotiation agreement
D. Mandatory free notice

---

### Q20. A fixed-price contract is BEST when:
A. Scope is unknown
B. Scope is clearly defined and stable
C. The supplier is new
D. The market is volatile

---

### Q21. e-RFx, e-auctions, and CLM are examples of:
A. Inventory tools
B. Forecasting tools
C. Procurement technology stack
D. Production scheduling tools

---

### Q22. A buyer with a strong BATNA can:
A. Make concessions to keep the supplier
B. Hold firm on terms because they have alternatives
C. Avoid all contracts
D. Skip negotiation

---

### Q23. In global sourcing, currency volatility is BEST mitigated by:
A. Ignoring exchange rates
B. FX hedging and multi-currency invoicing
C. Always paying in supplier currency
D. Single-currency policy regardless of risk

---

### Q24. A supplier rated "strategic" in Kraljic deserves:
A. Reverse auctions every 6 months
B. Deep partnership, multi-year contract, joint planning
C. Transactional ordering only
D. Mandatory price cuts

---

### Q25. A buyer for industrial bolts purchased in massive volume with many qualified global suppliers should pursue:
A. Strategic alliance
B. Competitive RFQs / reverse auctions
C. Vertical integration
D. Single-source long-term contract

---

## 🎯 Answers + Explanations

### Q1: **A. RFI**
RFI gathers market intel and capability data.

### Q2: **D. Strategic**
Custom + high impact + high risk → strategic.

### Q3: **B. Spend analysis**
Step 1. You can't strategize spend you don't understand.

### Q4: **B. Well-defined spec, price quotes**
RFQ = pure price competition on a known spec.

### Q5: **B. Operating, maintenance, disposal across lifecycle**
TCO captures the full lifecycle cost beyond price.

### Q6: **C. Strategic custom components**
Reverse auctions strip out the relationship that strategic items need.

### Q7: **D. Routine**
Low risk, low impact, high transaction volume → automate via e-catalog.

### Q8: **B. Ensure supply (dual-source, safety stock)**
Bottleneck strategy = supply security, not cost compression.

### Q9: **A. The buyer**
Cost-plus has the buyer absorbing cost overruns.

### Q10: **B. Service-Level Agreement**
SLA defines performance commitments and penalties.

### Q11: **A. Best Alternative To a Negotiated Agreement**
Your walk-away leverage in negotiation.

### Q12: **A. Quality and on-time delivery**
Dickson 1966 and modern scorecards both put these at the top.

### Q13: **B. Joint R&D, shared scorecards, equity**
Top of the SRM ladder = deepest collaboration.

### Q14: **B. Develop alternates and safety stock**
Bottleneck mitigation = de-risk supply.

### Q15: **B. TCO**
The four classic TCO buckets.

### Q16: **B. Competitive bidding / reverse auction**
High impact + low risk = use scale to drive price down.

### Q17: **D. Compute EOQ**
EOQ is an inventory technique, not a sourcing step.

### Q18: **B. Quality**
Quality has consistently ranked #1 since 1966.

### Q19: **A. Best pricing of any customer**
MFN guarantees price equality.

### Q20: **B. Defined and stable scope**
FP only makes sense when both parties can pre-price the work.

### Q21: **C. Procurement tech stack**
All three sit inside the modern source-to-pay platform family.

### Q22: **B. Hold firm on terms**
Strong BATNA = leverage to walk away if terms aren't met.

### Q23: **B. FX hedging + multi-currency**
The standard mitigation playbook for currency risk.

### Q24: **B. Deep partnership, multi-year, joint planning**
Strategic suppliers deserve relationship investment.

### Q25: **B. Competitive RFQs / reverse auctions**
Commodity + many suppliers + scale = leverage; use price competition.

---

## 📊 Score Yourself

- 23–25/25 → 🏆 Sourcing mastered.
- 20–22/25 → ✅ Strong. Review the misses.
- 15–19/25 → ⚠️ Re-read Kraljic + RFx sections.
- <15/25 → 🔁 Restart Reading.md.

---

## 🃏 Add To Your Flashcards

- 7 strategic sourcing steps
- Kraljic matrix 4 quadrants + strategies
- RFI vs RFP vs RFQ
- TCO buckets
- SRM maturity ladder
- BATNA definition
- Contract types (FP, cost-plus, T&M, IDIQ, MSA, performance)
- Reverse auction use cases

---

➡️ [Cheat-Sheet.md](./Cheat-Sheet.md) → [Module 7: Logistics, Distribution & Warehousing](../Module-07-Logistics-Distribution-Warehousing/Reading.md)
