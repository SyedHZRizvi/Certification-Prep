<link href="https://fonts.googleapis.com/css2?family=Noto+Nastaliq+Urdu:wght@400;500;600;700&family=Gulzar&display=swap" rel="stylesheet">
<style>
.lang-ur h1,.lang-ur h2,.lang-ur h3,.lang-ur h4{font-family:'Gulzar','Noto Nastaliq Urdu',serif;font-weight:400;line-height:2.4;}
.lang-ur h1{font-size:1.8em;}
.lang-ur h2{font-size:1.5em;}
.lang-ur h3{font-size:1.25em;}
</style>
<div class="lang-switcher" markdown="0" style="background:linear-gradient(135deg,#064e3b,#065f46);border-radius:12px;padding:1.25rem;margin:0 0 2rem;text-align:center;box-shadow:0 4px 16px rgba(16,185,129,0.3);">
  <p style="color:#d1fae5;font-size:1rem;font-weight:600;margin:0 0 0.85rem;">🌐 Choose your learning language / اپنی زبان منتخب کریں</p>
  <div style="display:flex;gap:0.75rem;justify-content:center;flex-wrap:wrap;">
    <button id="ls-btn-en" onclick="certHubSetLang('en')" style="padding:0.6rem 1.6rem;border-radius:9px;border:2px solid #34d399;background:#fff;color:#065f46;font-weight:700;font-size:0.95rem;cursor:pointer;">🇬🇧 English</button>
    <button id="ls-btn-ur" onclick="certHubSetLang('ur')" style="padding:0.6rem 1.6rem;border-radius:9px;border:2px solid #34d399;background:#fff;color:#065f46;font-weight:700;font-size:0.95rem;cursor:pointer;">🇵🇰 اردو (Urdu)</button>
  </div>
</div>
<script>
function certHubSetLang(l){try{localStorage.setItem('cert-hub-lang-pref',l);}catch(e){}document.querySelectorAll('.lang-en,.lang-ur').forEach(function(el){el.style.display='none';});document.querySelectorAll('.lang-'+l).forEach(function(el){el.style.display='block';});var be=document.getElementById('ls-btn-en'),bu=document.getElementById('ls-btn-ur');if(be&&bu){be.style.background=l==='en'?'#065f46':'#fff';be.style.color=l==='en'?'#fff':'#065f46';bu.style.background=l==='ur'?'#065f46':'#fff';bu.style.color=l==='ur'?'#fff':'#065f46';}}
(function(){var s='';try{s=localStorage.getItem('cert-hub-lang-pref')||'';}catch(e){}certHubSetLang(s||'en');})();
</script>

<div class="lang-en" markdown="1">

# Module 7 — Juz Amma: Short Surahs — Part 2

## 🌟 Five More Gems of Juz Amma

| Surah | Number | Verses | Theme |
|-------|--------|--------|-------|
| An-Nasr | 110 | 3 | Victory and gratitude — the last message |
| Al-Kafirun | 109 | 6 | The declaration of religious freedom |
| Al-Ma'un | 107 | 7 | The surah of social justice |
| Al-Quraysh | 106 | 4 | Gratitude for prosperity and security |
| Al-Fil | 105 | 5 | The army of elephants — divine protection |

---

## 📖 Surah An-Nasr — سُورَةُ النَّصر (110)

<div style="background:linear-gradient(135deg,#f0fdf4,#dcfce7);border:2px solid #34d399;border-radius:16px;padding:2rem;margin:1.5rem 0;text-align:center;">
<div style="font-size:2.2em;direction:rtl;line-height:3;font-family:'Amiri','Scheherazade New','Traditional Arabic',serif;color:#064e3b;">

بِسۡمِ ٱللَّهِ ٱلرَّحۡمَٰنِ ٱلرَّحِيمِ

إِذَا جَآءَ نَصۡرُ ٱللَّهِ وَٱلۡفَتۡحُ ﴿١﴾

وَرَأَيۡتَ ٱلنَّاسَ يَدۡخُلُونَ فِي دِينِ ٱللَّهِ أَفۡوَاجٗا ﴿٢﴾

فَسَبِّحۡ بِحَمۡدِ رَبِّكَ وَٱسۡتَغۡفِرۡهُۚ إِنَّهُۥ كَانَ تَوَّابَۢا ﴿٣﴾

</div>
</div>

### Verse-by-Verse

| Verse | Translation | Key Term |
|-------|-------------|---------|
| 1 | When Allah's help comes and the opening (of Mecca) | نَصر = help; فَتح = opening/victory (the conquest of Mecca) |
| 2 | And you see the people entering Allah's religion in multitudes | أَفوَاجاً = in multitudes, in crowds |
| 3 | Then exalt with praise of your Lord and ask His forgiveness — He is ever accepting of repentance | سَبِّح = glorify; استَغفِر = seek forgiveness; تَوَّاب = the ever-repentance-accepting |

**Why this surah moved people to tears:** Ibn Abbas (the Prophet's (sallallahu alayhi wa aalihi wasallam) cousin) narrated that when Umar ibn al-Khattab (ra) recited this surah to the Companions, he wept. They asked why. He said: "This surah tells the Prophet (sallallahu alayhi wa aalihi wasallam) that his mission is complete — that he will depart soon." When a task is finished and victory comes, what a wise person does is glorify Allah and seek forgiveness — not celebrate themselves.

**The last of the complete surahs revealed:** An-Nasr was the last complete surah to be revealed. After it, no entire surah was revealed — only scattered verses.

**Response to success:** The surah defines the correct response to victory: **tasbih** (glorification of Allah), **hamd** (praise), and **istighfar** (seeking forgiveness). Not pride, not celebration of oneself. The greater the victory, the more profound the humility.

---

## 📖 Surah Al-Kafirun — سُورَةُ الكَافِرُون (109)

<div style="background:linear-gradient(135deg,#f0fdf4,#dcfce7);border:2px solid #34d399;border-radius:16px;padding:2rem;margin:1.5rem 0;text-align:center;">
<div style="font-size:2.2em;direction:rtl;line-height:3;font-family:'Amiri','Scheherazade New','Traditional Arabic',serif;color:#064e3b;">

بِسۡمِ ٱللَّهِ ٱلرَّحۡمَٰنِ ٱلرَّحِيمِ

قُلۡ يَٰٓأَيُّهَا ٱلۡكَٰفِرُونَ ﴿١﴾

لَآ أَعۡبُدُ مَا تَعۡبُدُونَ ﴿٢﴾

وَلَآ أَنتُمۡ عَٰبِدُونَ مَآ أَعۡبُدُ ﴿٣﴾

وَلَآ أَنَا۠ عَابِدٞ مَّا عَبَدتُّمۡ ﴿٤﴾

وَلَآ أَنتُمۡ عَٰبِدُونَ مَآ أَعۡبُدُ ﴿٥﴾

لَكُمۡ دِينُكُمۡ وَلِيَ دِينِ ﴿٦﴾

</div>
</div>

### Verse-by-Verse

| Verse | Translation |
|-------|-------------|
| 1 | Say: O disbelievers |
| 2 | I do not worship what you worship |
| 3 | Nor do you worship what I worship |
| 4 | Nor will I ever be a worshipper of what you worship |
| 5 | Nor will you be worshippers of what I worship |
| 6 | For you is your religion, and for me is my religion |

**Why the repetition?** Verses 2-3 speak to the present: "I don't worship what you worship now." Verses 4-5 speak to the future: "I will not be a worshipper..." This is not redundancy — it covers both present refusal and permanent commitment.

**The context:** The Quraysh offered a deal to the Prophet (sallallahu alayhi wa aalihi wasallam): "You worship our gods for one year, we will worship your God for one year — a compromise." This surah is the divine response: there is no compromise in the object of worship. Religious coexistence does not require worship exchange.

**The declaration of religious freedom:** Verse 6 is one of the clearest statements of religious non-coercion in any scripture: "For you is your religion, for me is mine." This is not indifference — it is clarity about the absolute distinction.

---

## 📖 Surah Al-Ma'un — سُورَةُ المَاعُون (107)

<div style="background:linear-gradient(135deg,#f0fdf4,#dcfce7);border:2px solid #34d399;border-radius:16px;padding:2rem;margin:1.5rem 0;text-align:center;">
<div style="font-size:2.2em;direction:rtl;line-height:3;font-family:'Amiri','Scheherazade New','Traditional Arabic',serif;color:#064e3b;">

بِسۡمِ ٱللَّهِ ٱلرَّحۡمَٰنِ ٱلرَّحِيمِ

أَرَءَيۡتَ ٱلَّذِي يُكَذِّبُ بِٱلدِّينِ ﴿١﴾

فَذَٰلِكَ ٱلَّذِي يَدُعُّ ٱلۡيَتِيمَ ﴿٢﴾

وَلَا يَحُضُّ عَلَىٰ طَعَامِ ٱلۡمِسۡكِينِ ﴿٣﴾

فَوَيۡلٞ لِّلۡمُصَلِّينَ ﴿٤﴾

ٱلَّذِينَ هُمۡ عَن صَلَاتِهِمۡ سَاهُونَ ﴿٥﴾

ٱلَّذِينَ هُمۡ يُرَآءُونَ ﴿٦﴾

وَيَمۡنَعُونَ ٱلۡمَاعُونَ ﴿٧﴾

</div>
</div>

### Verse-by-Verse

| Verse | Translation | Key Term |
|-------|-------------|---------|
| 1 | Have you seen the one who denies the Day of Judgment? | يُكَذِّب = actively denies/rejects |
| 2 | That is the one who drives away the orphan | يَدُعُّ = pushes away, repels harshly |
| 3 | And does not encourage feeding the poor | — |
| 4 | So woe to those who pray | وَيل = woe/destruction |
| 5 | Who are heedless of their prayer | سَاهُون = neglectful, heedless |
| 6 | Those who pray to be seen | يُرَاءُون = perform for an audience |
| 7 | And withhold simple assistance | المَاعُون = small household items, simple neighborly help |

**The most astonishing surah on social justice:** This surah redefines "denial of the Day of Judgment" not as a theological statement but as a behavioral pattern. You don't have to say "I don't believe in Judgment Day" — if you push away orphans and ignore the poor, you are functionally living as if there is no accountability.

**The prayer warning (verses 4-5):** The surah does not criticize those who miss prayers due to difficulty — it criticizes those who pray but are *heedless* (sāhūn — mindless, going through motions without presence). And those who pray *to be seen* (riyā — performance, showing off).

**Al-Ma'un (simple goods):** The surah ends with the smallest possible act of generosity — lending neighbors basic household items. If someone refuses even this smallest act, they have failed the basic test of Islamic character.

---

## 📖 Surah Al-Quraysh — سُورَةُ قُرَيش (106)

<div style="background:linear-gradient(135deg,#f0fdf4,#dcfce7);border:2px solid #34d399;border-radius:16px;padding:2rem;margin:1.5rem 0;text-align:center;">
<div style="font-size:2.2em;direction:rtl;line-height:3;font-family:'Amiri','Scheherazade New','Traditional Arabic',serif;color:#064e3b;">

بِسۡمِ ٱللَّهِ ٱلرَّحۡمَٰنِ ٱلرَّحِيمِ

لِإِيلَٰفِ قُرَيۡشٍ ﴿١﴾

إِۦلَٰفِهِمۡ رِحۡلَةَ ٱلشِّتَآءِ وَٱلصَّيۡفِ ﴿٢﴾

فَلۡيَعۡبُدُواْ رَبَّ هَٰذَا ٱلۡبَيۡتِ ﴿٣﴾

ٱلَّذِيٓ أَطۡعَمَهُم مِّن جُوعٖ وَءَامَنَهُم مِّنۡ خَوۡفٍ ﴿٤﴾

</div>
</div>

| Verse | Translation |
|-------|-------------|
| 1-2 | For the accustomed security of Quraysh — their accustomed security in their winter and summer journeys |
| 3 | So let them worship the Lord of this House |
| 4 | Who has fed them against hunger and made them secure against fear |

**Context:** The Quraysh had trade caravans travelling to Yemen (winter) and Syria (summer). They were protected and respected because of their role as guardians of the Kaaba. The argument: Allah gave you all this prosperity (food, safety, trade). Your obligation is gratitude through worship.

**Connected to Surah Al-Fil:** Some scholars read surahs 105 (Al-Fil) and 106 (Al-Quraysh) as one continuous argument. Al-Fil: "We protected you from the elephant army." Al-Quraysh: "And We gave you these trade blessings — so worship Us."

---

## 📖 Surah Al-Fil — سُورَةُ الفِيل (105)

<div style="background:linear-gradient(135deg,#f0fdf4,#dcfce7);border:2px solid #34d399;border-radius:16px;padding:2rem;margin:1.5rem 0;text-align:center;">
<div style="font-size:2.2em;direction:rtl;line-height:3;font-family:'Amiri','Scheherazade New','Traditional Arabic',serif;color:#064e3b;">

بِسۡمِ ٱللَّهِ ٱلرَّحۡمَٰنِ ٱلرَّحِيمِ

أَلَمۡ تَرَ كَيۡفَ فَعَلَ رَبُّكَ بِأَصۡحَٰبِ ٱلۡفِيلِ ﴿١﴾

أَلَمۡ يَجۡعَلۡ كَيۡدَهُمۡ فِي تَضۡلِيلٖ ﴿٢﴾

وَأَرۡسَلَ عَلَيۡهِمۡ طَيۡرًا أَبَابِيلَ ﴿٣﴾

تَرۡمِيهِم بِحِجَارَةٖ مِّن سِجِّيلٖ ﴿٤﴾

فَجَعَلَهُمۡ كَعَصۡفٖ مَّأۡكُولِۭ ﴿٥﴾

</div>
</div>

| Verse | Translation | Key Term |
|-------|-------------|---------|
| 1 | Did you not see what your Lord did to the People of the Elephant? | أصحاب الفيل = Abraha's army from Yemen |
| 2 | Did He not make their plan go astray? | كَيدَهُم = their plot/scheme |
| 3 | And He sent against them birds in flocks | طَيراً أَبَابِيل = birds in flocks/groups |
| 4 | Pelting them with stones of hard clay | سِجِّيل = hard dried clay |
| 5 | And He made them like consumed dry grass | عَصف مَأكُول = eaten/hollow straw |

**The Year of the Elephant (570 CE):** This event occurred in the year the Prophet (sallallahu alayhi wa aalihi wasallam) was born. The Abyssinian governor Abraha marched toward Mecca with an army including war elephants, intending to destroy the Kaaba. According to the narration, swarms of birds dropped stones and the army was destroyed. Mecca and the Kaaba were preserved.

**The question format:** "Did you not see?" is a rhetorical question — it means "This is clearly visible history." The people of Mecca knew this event; it had just happened a generation before.

---

## 📋 Summary

| Surah | Core Message |
|-------|-------------|
| An-Nasr (110) | Victory comes from Allah — respond with humility, not pride |
| Al-Kafirun (109) | No compromise in worship — "To you your religion, to me mine" |
| Al-Ma'un (107) | Real faith is measured by treatment of orphans and the poor |
| Al-Quraysh (106) | Prosperity and security are divine gifts that demand worship |
| Al-Fil (105) | Allah protects the Kaaba — no power can overcome divine will |

</div>

<div class="lang-ur" style="direction:rtl;text-align:right;font-family:'Noto Nastaliq Urdu','Gulzar','Urdu Typesetting',serif;line-height:2.2;font-size:1.1rem;" markdown="1">

# ماڈیول ۷ — جُزء عَمَّ: چھوٹی سورتیں — حصہ دوم

## سورۃ النصر

> إِذَا جَآءَ نَصۡرُ ٱللَّهِ وَٱلۡفَتۡحُ ۔ وَرَأَيۡتَ ٱلنَّاسَ يَدۡخُلُونَ فِي دِينِ ٱللَّهِ أَفۡوَاجٗا ۔ فَسَبِّحۡ بِحَمۡدِ رَبِّكَ وَٱسۡتَغۡفِرۡهُ

ترجمہ: جب اللہ کی مدد اور فتح آجائے، اور تم لوگوں کو اللہ کے دین میں فوج در فوج داخل ہوتے دیکھو — تو اپنے رب کی تعریف کے ساتھ تسبیح کرو اور اس سے استغفار کرو۔

یہ آخری مکمل سورت ہے جو نازل ہوئی۔ فتح کے بعد تکبر نہیں — تسبیح و استغفار۔

---

## سورۃ الکافرون

> قُلۡ يَٰٓأَيُّهَا ٱلۡكَٰفِرُونَ ۔ لَآ أَعۡبُدُ مَا تَعۡبُدُونَ ۔ وَلَآ أَنتُمۡ عَٰبِدُونَ مَآ أَعۡبُدُ ۔ لَكُمۡ دِينُكُمۡ وَلِيَ دِينِ

ترجمہ: کہو اے کافرو! میں اس کی عبادت نہیں کرتا جس کی تم کرتے ہو — تمہارا دین تمہارے لیے اور میرا دین میرے لیے۔

قریش نے سمجھوتے کی پیشکش کی — آپ ایک سال ہمارے بتوں کی پوجا کریں، ہم ایک سال آپ کے اللہ کی۔ یہ سورت اس تجویز کا جواب ہے۔

---

## سورۃ الماعون

ترجمہ: کیا آپ نے اسے دیکھا جو روزِ جزا کو جھٹلاتا ہے؟ وہی تو یتیم کو دھکیلتا ہے، اور مسکین کو کھانا کھلانے کی ترغیب نہیں دیتا۔ تو ان نمازیوں کے لیے ویل ہے جو اپنی نماز سے بے خبر ہیں، جو ریاکاری کرتے ہیں اور معمولی استعمال کی چیز دینے سے بھی گریز کرتے ہیں۔

قرآن کی سب سے اہم سماجی انصاف کی سورت — نماز پڑھنے والوں کو بھی تنبیہ ہے اگر وہ ریاکار ہوں یا یتیم کو دھتکاریں۔

---

## سورۃ القریش

ترجمہ: قریش کو مانوس رکھنے کی وجہ سے — جاڑے اور گرمی کے سفر میں ان کے مانوس ہونے کی وجہ سے — لہٰذا وہ اس گھر کے رب کی عبادت کریں جس نے انہیں بھوک سے بچایا اور خوف سے امن دیا۔

---

## سورۃ الفیل

ترجمہ: کیا آپ نے نہیں دیکھا آپ کے رب نے ہاتھی والوں کے ساتھ کیا کیا؟ کیا اس نے ان کی چال کو بے کار نہیں کردیا؟ اور ان پر پرندوں کے غول بھیجے جو انہیں پختہ مٹی کے پتھر مارتے تھے اور انہیں کھائی ہوئی گھاس جیسا کردیا؟

ابرہہ کا ہاتھیوں کا لشکر — رسول اللہ صلی اللہ علیہ وآلہ وسلم کی ولادت کے سال کا واقعہ — اللہ نے کعبے کی حفاظت فرمائی۔

</div>
