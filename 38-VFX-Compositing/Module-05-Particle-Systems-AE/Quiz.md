---
title: "Module 5 Quiz: Particle Systems in After Effects"
---

# 🧪 Module 5 Quiz: Particle Systems in After Effects
...
In a particle system, what is the "emitter"?

A. The physics engine that calculates forces on particles
B. The source location where particles are spawned into the scene
C. The parameter that controls how fast particles die
D. The blend mode applied to the particle layer

---

### Q2.

In Trapcode Particular, which parameter controls how many new particles are created per second?

A. Life
B. Velocity
C. Particles/sec (Emitter → Particles/sec)
D. Birth Rate
...
When building a fire effect in Trapcode Particular, what does a negative Gravity Y value (e.g., −200) cause?

A. Particles to fall toward the ground
B. Particles to be repelled from the emitter
C. Particles to rise upward (against the normal pull of gravity)
D. Particles to spin in a circular motion

---

### Q4.

The "Air Turbulence" parameter in Trapcode Particular is most useful for which type of effect?

A. A perfectly uniform stream of bubbles rising in water
B. Fire flickering, smoke billowing, or particles swirling in an unpredictable way
C. A fixed beam of light particles traveling in a straight line
D. Particles that follow a predefined path curve

---

### Q5.

What is the Trapcode Particular "Aux System"?

A. A secondary physics engine that overrides the primary physics
B. An additional emitter that spawns secondary particles from the location where primary particles die
C. A separate rendering layer for the particle system
D. The system for importing custom 3D shapes as particle sprites

---

### Q6.

Which of the following is an advantage of CC Particle World over Trapcode Particular?

A. CC Particle World has more physics parameters
B. CC Particle World supports custom OBJ models as particles
C. CC Particle World is built into After Effects and requires no additional purchase
D. CC Particle World renders faster in all scenarios

---

### Q7.

In the Fractal Noise fire method (no plugins), what does animating the "Evolution" parameter over time achieve?

A. The fire grows in size over time
B. The fractal noise pattern changes, simulating the movement and flickering of flames
C. The color of the fire shifts from red to yellow
D. The fire fades out at the end of the clip

---

### Q8.

When building a smoke particle system in Trapcode Particular, which combination of physics settings best produces the billowing, expanding behavior of real smoke?

A. High Gravity (positive), low turbulence
B. Low negative Gravity (e.g., −30), high Air Turbulence with large Turbulence Scale
C. Zero Gravity, high Velocity, low Spread
D. High Air Resistance, high Velocity, no Turbulence

---

### Q9.

The "Life" parameter in Trapcode Particular controls what?

A. The color of a particle at birth
B. How long a particle exists before fading out and being removed from the simulation
C. The maximum speed a particle can travel
D. The number of particles that can exist simultaneously

---

### Q10.

For a dust cloud kicked up by an explosion, which Emitter Type in Trapcode Particular is most appropriate?

A. Point (single emission point)
B. Sphere
C. Box (emission from a wide rectangular volume near the ground)
D. Layer (emission from a specific image layer)
...
When compositing a fire effect over live-action footage in After Effects, which blend mode is most commonly used for fire particle layers?
...
In CC Particle World, the "Longevity" parameter controls what?

A. The number of frames the particle simulation runs
B. How long each individual particle lives before it is removed
C. The persistence of the particle system's motion blur
D. The length of trail/streak particles

---

### Q14.

For a magic portal effect (like Dr. Strange's rings), which emitter type in Trapcode Particular would most efficiently emit particles along the edge of a ring shape?

A. Point emitter at the center of the ring
B. Sphere emitter with a very small radius
C. Layer emitter using a ring-shaped image as the emission source
D. Box emitter with a very thin height
...
In the Fractal Noise fire method, what is the purpose of applying a Curves adjustment to the fractal noise output?
...
Which particle type in CC Particle World produces soft, blurred spheres that are excellent for bokeh/out-of-focus light particle effects?
...
A particle system emitting fire has particles that look too uniform and "CG." Which Trapcode Particular parameters should be adjusted to increase visual variety?

A. Reduce the Life value to 0.1 seconds
B. Increase Life Random, Velocity Random, and Size Random to introduce variation per particle
C. Decrease the Particles/sec value
D. Change the Emitter Type from Sphere to Point

---

### Q19.

When building a layered fire effect in After Effects, what is the recommended compositing order of layers (bottom to top)?

A. Smoke → Outer Flame → Core Flame → Embers (embers on top)
B. Embers → Smoke → Outer Flame → Core Flame
C. Core Flame → Outer Flame → Smoke → Embers (embers on bottom)
D. The order doesn't matter for fire effects
...
When using CC Particle World to make particles appear in 3D space (depth), which parameter positions the emitter along the Z axis (depth)?

A. Producer Position Z
B. Gravity Z
C. Velocity Z
D. Floor Z Position
...
A compositor wants fire particles to throw light onto a live-action scene below them in the composition. Which is the most practical technique in After Effects?
...
In Trapcode Particular, the "Opacity over Life" parameter allows what specific behavior in a smoke particle system?

A. Making all particles fully opaque throughout their lifetime
B. Having particles start semi-transparent at birth, reach maximum opacity at midlife, then fade to transparent at death
C. Animating the opacity of the particle emitter layer
D. Controlling how quickly light passes through the smoke

---

### Q24.

Which combination of Trapcode Particular parameters produces a convincing falling ember (a bright spark that launches upward then falls and tumbles):

A. Low Velocity, Positive Gravity, High Spin, Long Life
B. High Velocity upward, Positive Gravity, High Spin random, Medium Life (2–3s), Embers sprite
C. High Gravity negative, Sprite = smoke cloud, Life = 8s
D. Box Emitter, Zero Gravity, Air Resistance = 1.0, Particle Type = Line
